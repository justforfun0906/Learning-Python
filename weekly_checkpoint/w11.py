...
class Records:
    ...
    def delete(self, in_str):
        try:
            ...
            in_str = in_str.strip()
            assert not(Categories.is_valid_integer(in_str)), 'Invalid format. Description is not a string, Fail to delete a record.'
        except AssertionError as e:
            print(e)
            return
        candidata = []
    ...
    def find(self, in_str):
        subcate = Categories.find_subcate(in_str, Records.category_list)
        #output
        print("Category        Description          Amount")
        print("=============== ==================== ======")
        for record in self._records:
...
class Categories:
    ...
    def __init__(self):
        ...
        self._flatten_category_list = Categories.flatten_list(self._category_list)
    ...
    def find_subcate(category, categories):
        if type(categories) == list:
            for v in categories:
                result = Categories.find_subcate(category, v)
                if result == True:
                    index = categories.index(v)
                    if index + 1 < len(categories) and \
                            type(categories[index + 1]) == list:
                        return Categories.flatten_list(categories[index:index + 2])
                    else:
    ...
    def flatten_list(li):
        if type(li) == list:
            ret_val = []
            for item in li:
                ret_val.extend(Categories.flatten_list(item))
            return ret_val
        else:
            return [li]
